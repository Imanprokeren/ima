	...
	case 'inspect':
		...
		*Id desc* : ${descId}
		...
		var groupz = await alpha.chats.all().filter(v => v.jid.endsWith('g.us'))
				...
				.all().filter(v => v.jid.endsWith('g.us'))
                    reply(`\`\`\`[ ! ]\`\`\` Broadcast in progress! Total: ${groupz.length} groups`)
                    await ini_bc_gc_bang(ini_bc)
					reply(`\`\`\`[ ✓ ] Success broadcast : ${groupz.length} groups\`\`\``)
					break
...
sendButImage(from,  `Nih Udah jdi, Jgn Lupa donasi Ya! ©ImanBot 🐦 ` , `© ${ownername}`,photooxy2, [{"buttonId": `owner`,"buttonText": {"displayText": "👨‍💻 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "💰 Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
